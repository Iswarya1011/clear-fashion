...
selectRecent.addEventListener('change', async (event) => {
  ...
  if(event.target.value == "Yes"){
    ...
    products = Filterbydate(products);
...
selectSort.addEventListener('change', async (event) => {
  ...
  if(event.target.value =="price-asc"){
    ...
    products = SortPricesA(products);
  }
    
  else if(event.target.value == "price-desc"){

    products =SortPricesD(products);
    ...
    products =SortDateR(products);
    
  }
  else if(event.target.value == "date-desc"){

    products =SortDateO(products);
  }
  setCurrentProducts(products);
  render(currentProducts, currentPagination);
});
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  brands.unshift("All the products");
  ...
  spanNbNewProducts.innerHTML=pro.filter(product=> (((new Date()- new Date (product.released)) / (1000 * 3600 * 24 )) <30) ).length;
 

  spanRelease.innerHTML= pro.sort((a,b)=> (new Date(b.released)-new Date(a.released)) )[0].released;
  const price=[];

  for(let step = 0; step < currentPagination.count; step++){

    price.push(pro[step].price);
